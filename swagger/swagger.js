...
const doc = {
  info: {
    ...
    description:
      ...
      '## Authentication\n\n' +
      '### ➡️ [Sign in with GitHub](/auth/github)\n\n' +
      'GET routes are public. Every POST, PUT and DELETE requires a sign-in. Use the link ' +
      'above - it navigates this tab to GitHub and brings you straight back here with a ' +
      'session cookie, which Try it out then reuses automatically on the protected routes.\n\n' +
      'Do **not** press Execute on `GET /auth/github`: that route answers with a redirect to ' +
      'github.com, and a browser will not let this page\'s JavaScript follow a redirect to ' +
      'another site, so it reports "Failed to fetch". The route is working - it simply has to ' +
      'be followed by the browser itself, which is what the link above does.\n\n' +
      'Check [/auth/status](/auth/status) to see who you are signed in as, and ' +
      '[/auth/logout](/auth/logout) to end the session. Signed out, protected routes answer 401.'
  },
